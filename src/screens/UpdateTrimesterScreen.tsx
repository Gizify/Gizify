...
interface UpdateTrimesterScreenProps {}

const UpdateTrimesterScreen: React.FC<UpdateTrimesterScreenProps> = () => {
  ...
  useEffect(() => {
    ...
    if (user?.medical_history) {
      setHealthHistory(user.medical_history);
    }
  }, [user?.height]);
  ...
  const handleSubmit = async () => {
    if (!height || !weight || !activity || !healthHistory) {
    ...
    try {
      ...
      const profileData = {
        ...
        medical_history: Array.isArray(healthHistory) ? healthHistory : healthHistory === "Tidak ada" ? [] : [healthHistory],
      };
  ...
  const isButtonDisabled = !height || !weight || !activity || !healthHistory;
  ...
  return (
    <ScrollView contentContainerStyle={styles.container}>
      ...
      <View style={styles.row}>
        <TextInput placeholder="Tinggi Badan (cm) *" style={styles.inputHalf} keyboardType="numeric" value={height !== null ? String(height) : ""} onChangeText={(text) => setHeight(Number(text))} />
        <TextInput placeholder="Berat Badan (kg) *" style={styles.inputHalf} keyboardType="numeric" value={weight !== null ? String(weight) : ""} onChangeText={(text) => setWeight(Number(text))} />
      </View>
...
export default UpdateTrimesterScreen;
